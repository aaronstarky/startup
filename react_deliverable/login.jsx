...
export default function Login() {
    async function submitLoginRequest() {
        ...
        const encodedUrl = encodeURI(`http://localhost:4000/api/auth/login/${email}/${password}`);
    ...
    async function submitRegisterRequest() {
        const email = document.getElementById("email").value;
        const password = document.getElementById("password").value;
        const encodedUrl = encodeURI(`http://localhost:4000/api/auth/register/${email}/${password}`);
        const response = await fetch(encodedUrl, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
